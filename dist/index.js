...
function ensureWasm() {
    if (wasmReady === null) {
        wasmReady = import("../pkg/schemasniff.js").then((m) => {
            wasmModule = m;
        }).catch((err) => {
...
let isRunning = false;
// ── Raw WASM call ─────────────────────────────────────────────────────────────
function callWasm(input) {
    try {
        const raw = wasmModule.infer_schema(input);
    ...
    for (let i = 1; i < results.length; i++) {
        ...
        for (const col of r.columns) {
            ...
            if (!acc) {
                ...
                if (mn !== null)
                    acc.numeric_min = acc.numeric_min === null ? mn : Math.min(acc.numeric_min, mn);
                if (mx !== null)
                    acc.numeric_max = acc.numeric_max === null ? mx : Math.max(acc.numeric_max, mx);
...
async function runChunked(chunks, onProgress) {
    const results = [];
    ...
    if (results.length === 1)
        return { ...results[0], chunk_count: 1 };
    return mergeResults(results, chunks.length);
}
// ── Public API ────────────────────────────────────────────────────────────────
export async function inferSchema(input, options) {
